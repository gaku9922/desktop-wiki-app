...
# 共有リンク作成機能（実装済み・スコープA）

記事の ⋯ メニュー「共有リンクを作成」から、カスタムURLスキームの共有リンクを表示・コピーできる。
インストール済み環境では、そのリンクを開くとアプリが起動し該当記事を表示する。

## リンク形式
- `ugbwiki://article/<記事ID>`（例 `ugbwiki://article/UGB0001`）
- 共有Wiki（同一ファイルサーバ）前提のため記事IDは全員共通 → 受信側の環境で同じ記事が開く

## 実装（メインプロセス）
- `app.setAsDefaultProtocolClient('ugbwiki')` でスキーム登録（dev は execPath+args を明示）
- **単一インスタンス化**（`requestSingleInstanceLock` ＋ `second-instance`）。起動中に開かれた場合は
  既存ウィンドウをフォーカスして該当記事へ遷移
- 受信経路: macOS=`open-url` / Windows・Linux=起動引数（`process.argv`）。コールドスタートも対応
- 受信URLは `ugbwiki://article/UGB\d+` を**厳格に検証**し `#/article/<ID>` へ変換。
  ウィンドウ未準備時は保留し `did-finish-load` 後に反映
- レンダラーへは `app:navigate` で push（preload `onDeepLink`）→ 既存ハッシュルーティングで遷移

## UI・コピー
- ⋯ メニュー「共有リンクを作成」→ モーダルでリンク表示（読み取り専用）＋「コピー」
- コピーは Electron `clipboard.writeText`（IPC `clip:write`）で確実に

## 留意点（スコープA）
- 「リンク→アプリ起動」の OSレベルの動作は**パッケージ化／インストール後**に有効。
  現状は dev 起動（`electron .`）のためこの環境では動作確認不可（プロトコルハンドラのコードは実装済み）。
  リンクの生成・表示・コピーは dev でも動作する。
